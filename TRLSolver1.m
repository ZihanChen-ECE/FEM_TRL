...
mur = 1;
mu = pi*4e-7;
omega = [10e6,100e6,1e9,10e9,100e9,1e12];
sigmaAL2O3 = 3.6e7;
...
for ii = 1:Nelem
    ...
    %conductor 4 is set to be the ground
    if ((maxX<=23e-6 && minX>=12e-6) && (minY>=9e-6 && maxY<=10e-6)) %conductor 4
            sigma = sigmaAL2O3;
             for j = 1:Nvertex
                Eb(j) = EArea/3*mu*sigma*Eimp*(-1/11);
...
while Z > 0
 ...
 unew = zeros(length(u)-length(Bnode),3);
    ...
    for is = 1:3
        %if(is == 1) %Calculate I1,I2,I3,I4 excited by source is(on conductor is)
            if ((maxX<=16e-6 && minX>=15e-6) && (minY>=11e-6 && maxY<=12e-6))
                ...
                CPhi = sum(Ne(:).*u(ENode(:),is));
                I(1,is) = I(1,is) + -i*omega(io)*sigma*CPhi*EArea;       
            end
            if ((maxX<=18e-6 && minX>=17e-6) && (minY>=11e-6 && maxY<=12e-6))
                ...
                CPhi = sum(Ne(:).*u(ENode(:),is));
                I(2,is) = I(2,is) + -i*omega(io)*sigma*CPhi*EArea;       
            end
            if ((maxX<=20e-6 && minX>=19e-6) && (minY>=11e-6 && maxY<=12e-6))
                ...
                CPhi = sum(Ne(:).*u(ENode(:),is));
                I(3,is) = I(3,is) + -i*omega(io)*sigma*CPhi*EArea;       
            end
...
%                      end
%                 CPhi = sum(Ne(:).*u(ENode(:),is));
%                 I(4,is) = I(4,is) + -i*omega(io)*sigma*CPhi*EArea;       
%             end
