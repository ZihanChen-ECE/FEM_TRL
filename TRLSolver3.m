...
for ii = 1:Nelem
    ...
    if (((maxX<=26e-6 && minX>=25e-6) && (minY>=18e-6 && maxY<=19e-6))||((maxX<=28e-6 && minX>=27e-6) && (minY>=18e-6 && maxY<=19e-6))...
        ||((maxX<=30e-6 && minX>=29e-6) && (minY>=18e-6 && maxY<=19e-6))||((maxX<=33e-6 && minX>=22e-6) && (minY>=16e-6 && maxY<=17e-6)))
        sigma = sigmaAL2O3;
    else
        sigma = 0;
    end
    ...
    %excite conductors
    for is = 1:4
        if ((maxX<=26e-6 && minX>=25e-6) && (minY>=18e-6 && maxY<=19e-6) && is == 1) % conductor 1
            sigma = sigmaAL2O3;
            for j = 1:Nvertex
        ...
        if ((maxX<=28e-6 && minX>=27e-6) && (minY>=18e-6 && maxY<=19e-6) && is == 2) %conductor 2
            sigma = sigmaAL2O3;
            for j = 1:Nvertex
                Eb(j) = EArea/3*mu*sigma*Eimp;
                b(ENode(j),is) = b(ENode(j),is) + Eb(j);
        ...
        if ((maxX<=30e-6 && minX>=29e-6) && (minY>=18e-6 && maxY<=19e-6) && is == 3) %conductor 3
            sigma = sigmaAL2O3;
            for j = 1:Nvertex
                Eb(j) = EArea/3*mu*sigma*Eimp;
                b(ENode(j),is) = b(ENode(j),is) + Eb(j);
        ...
        if ((maxX<=33e-6 && minX>=22e-6) && (minY>=16e-6 && maxY<=17e-6) && is == 4) %conductor 4
            sigma = sigmaAL2O3;
            for j = 1:Nvertex
                Eb(j) = EArea/3*mu*sigma*Eimp;
                b(ENode(j),is) = b(ENode(j),is) + Eb(j);
...
for ii = 1:node_num
       if node_xT(ii,2)==0 || node_xT(ii,1)==0 || node_xT(ii,2)==38e-6 || node_xT(ii,1)==55e-6 %|| node_xT(i,2)==1%Depends on the scope of the region
               Bnode(Bm) = ii;
               Bm = Bm+1;
       end
...
for ii = 1:Nelem
    ...
    minY = min(Ey(:));
        ...
        if ((maxX<=26e-6 && minX>=25e-6) && (minY>=18e-6 && maxY<=19e-6))
            for is = 1:4
                Ne = zeros(3,1);
                     for j = 1:3
            ...
            if ((maxX<=28e-6 && minX>=27e-6) && (minY>=18e-6 && maxY<=19e-6))
                for is = 1:4
                    Ne = zeros(3,1);
                         for j = 1:3
                            Ne(j) = 1/2/EArea*(ae(j)+be(j)*Cx+ce(j)*Cy); 
            ...
            if ((maxX<=30e-6 && minX>=29e-6) && (minY>=18e-6 && maxY<=19e-6))
                for is = 1:4
                    Ne = zeros(3,1);
                         for j = 1:3
                            Ne(j) = 1/2/EArea*(ae(j)+be(j)*Cx+ce(j)*Cy); 
            ...
            if ((maxX<=33e-6 && minX>=22e-6) && (minY>=16e-6 && maxY<=17e-6))
                for is = 1:4
                    Ne = zeros(3,1);
                         for j = 1:3
                            Ne(j) = 1/2/EArea*(ae(j)+be(j)*Cx+ce(j)*Cy); 
